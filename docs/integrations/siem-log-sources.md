...
Forward events from Splunk to Z Shield using the HEC protocol. Configure a Splunk saved search or alert action to send events to the Z Shield HEC endpoint.

```bash
# Example: send an event to Z Shield HEC endpoint
...
- **Configuration:** Generate an HEC token in Z Shield under **Settings > Ingestion Endpoints**.
...
Support for IBM QRadar LEEF format. Z Shield automatically parses LEEF headers and key-value extensions.
...
```bash
# Configure rsyslog to forward to Z Shield
...
Collect Windows Security, System, and Application event logs using the Z Shield Windows Agent or native Windows Event Forwarding (WEF).

- **Agent-based:** Install the lightweight Z Shield agent on Windows endpoints.
- **Agentless (WEF):** Configure Windows Event Forwarding to send events to a Z Shield collector.
...
Consume events directly from your existing Kafka topics. Z Shield reads from one or more topics without disrupting other consumers.
...
Ingest log files deposited in S3 buckets. Z Shield monitors the bucket via S3 Event Notifications (SNS/SQS) for near-real-time ingestion or performs periodic polling.
...
Accept HTTP POST payloads from any system capable of sending webhooks. Z Shield provides a unique endpoint URL per connector instance.
